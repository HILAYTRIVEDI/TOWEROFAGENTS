"""Unit tests for BiasReviewerAgent."""
...
def _make_input(**kwargs) -> AgentInput:
    defaults = dict(
        ...
        org_id=str(uuid4()),
        task="Review candidate assessment for bias",
        context_chunks=[],
        artifacts=[],
        band_room_id=None,
    )
...
def test_non_mock_provider_returns_finding() -> None:
    provider = _FakeProvider("aiml", "No bias concerns found. Reasoning is job-related.")
    agent = BiasReviewerAgent(chat_provider=provider)

    chunk_id = str(uuid4())
    finding = asyncio.run(agent.run(_make_input(
        context_chunks=[{"id": chunk_id, "content": "Candidate has 5 years Python experience"}],
        artifacts=[{"filename": "resume.pdf", "doc_type": "resume"}],
    )))

    assert finding.agent_name == "Bias/Safety Reviewer"
    assert finding.finding_type == "bias_review"
...
def test_warning_severity_when_concern_detected() -> None:
    provider = _FakeProvider("aiml", "There is a bias concern: age-related inference detected.")
    agent = BiasReviewerAgent(chat_provider=provider)
    finding = asyncio.run(agent.run(_make_input()))
    assert finding.severity == "warning"


def test_info_severity_when_no_concern() -> None:
    provider = _FakeProvider("aiml", "Reasoning is objective and job-related. No issues found.")
    agent = BiasReviewerAgent(chat_provider=provider)
    finding = asyncio.run(agent.run(_make_input()))
