...
Retrieval must filter by `org_id` and include both the requested `workflow_id` and organization-shared chunks where `workflow_id` is null. The base scaffold defines parser/chunker/retriever contracts only; it does not generate embeddings or access Supabase.
